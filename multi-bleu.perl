...
#      print "s_ <=> $reference\n";
  $reference = lc($reference) if $lowercase;
	...
	if ($diff < $closest_diff) {
	    ...
	    # print STDERR "s:closestdiff".abs(length_translation_this_sentence-length)."=abs(length_translation_this_sentence-length),settinglen:closest_length\n";
	...
	for(my $n=1;$n<=4;$n++) {
		...
		if (!defined($REF_NGRAM{$ngram}) ||
		    ...
		    $REF_NGRAM{$ngram} = $REF_NGRAM_N{$ngram};
#	    print "i:REFNGRAM$ngram=REF_NGRAM{$ngram}<BR>\n";
		}
	    }
	}
    }
	...
	foreach my $ngram (keys %T_NGRAM) {
	    ...
	    my $n = $1;
            # my $corr = 0;
#	print "iengram Extra open brace or missing close bracengram}<BR>\n";
	    $TOTAL[$n] += $T_NGRAM{$ngram};
	    if (defined($REF_NGRAM{$ngram})) {
		if ($REF_NGRAM{$ngram} >= $T_NGRAM{$ngram}) {
		    $CORRECT[$n] += $T_NGRAM{$ngram};
                    # corr=T_NGRAM{$ngram};
#	    print "iecorrect1T_NGRAM{$ngram}<BR>\n";
		}
		else {
		    $CORRECT[$n] += $REF_NGRAM{$ngram};
                    # corr=REF_NGRAM{$ngram};
#	    print "iecorrect2REF_NGRAM{$ngram}<BR>\n";
		}
	    }
            # REFNGRAM$ngram=0if!definedREF_NGRAM{$ngram};
            # print STDERR "Extra open brace or missing close braces, REFNGRAM$ngram,T_NGRAM{Extra close brace or missing open bracecorr}\n"
	}
    }
    $s++;
}
